...
{{< figure src=`tildagon.webp` caption=`My Tildagon when I just received and assembled it` >}}
...
{{< figure src=`app-in-the-list.webp` caption=`App as it's shown in the list of apps in the main menu of the badge` >}}
...
{{< figure src=`app-1.webp` caption=`App displaying "Known Error" text` >}}
...
{{< figure src=`app-2.webp` caption=`App displaying "Unknown Error" text` >}}
...
{{< figure src=`app-3.webp` caption=`App in the glitch mode, displaying "Known Error" text (the pattern changes every 2 seconds)` >}}
